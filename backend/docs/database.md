...
**RLS с таким контекстом доверяет серверу.** Клиенту нельзя выдавать учётные данные БД или разрешать самостоятельно устанавливать `workspace_id`. Item API проверяет членство внутри каждой транзакции и разрешает запись только `owner`/`editor`. Временно он работает только с явно включённой серверной dev-identity; workspace или user ID не принимаются от клиента. Без `FLARE_DEV_MODE=true` маршруты закрыты. Этот режим предназначен только для локальной разработки и должен быть заменён проверенной identity из будущей авторизации. Приглашения, создание рабочих пространств пользователями и управление участниками ещё не реализованы.

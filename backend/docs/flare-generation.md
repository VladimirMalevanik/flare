...
old rows and migrations 0001–0005 remain intact. `insight_sources.quote` stores
...
generation. SECURITY DEFINER functions belong to the existing dedicated NOLOGIN
executor, use qualified objects and fixed `pg_catalog,public,pg_temp` search path,
...
python backend/scripts/check_flares_migration.py --pg-bin /path/to/postgresql/bin
python -m pip check
node --test frontend/tests/flare-provider.test.cjs
...
The older migration checker remains pinned to 0004 → 0005.

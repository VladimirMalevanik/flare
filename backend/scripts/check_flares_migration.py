    ...
    with tempfile.TemporaryDirectory(prefix='flare-flares-upgrade-', dir='/tmp') as root:
        ...
        try:
            ...
            if verify_analysis_runs:
                ...
                with psycopg.connect(dsn) as conn:
                    assert snapshot(conn) == preserved
                    assert conn.execute('SELECT version_num FROM alembic_version').fetchone() == ('0015',)
                ...
                print(f'PASS ({args.provider}): 0007 -> 0015; all existing data preserved; repeat upgrade; email, GitHub, analytics, import provenance, daily-analysis RLS and worker isolation')

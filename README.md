...
`GET /items/{id}`, optimistic `PATCH /items/{id}` и `DELETE /items/{id}`.
...
chunks без запуска ИИ. Один и тот же активный файл не создаёт
дубликат в пределах workspace. URL сохраняется как источник. Прямой audio-placeholder
отклоняется: голос станет доступен только через проверенный transcription flow после
явного согласия на передачу аудио провайдеру.

Flares загружаются из БД. ИИ запускается отдельным пользовательским действием или
расписанием после сохранения свежих источников. `activity_events` считает создание,
редактирование, импорт, Analyze, расписание и GitHub connection flow без текста заметок,
названий репозиториев, OAuth-кодов или содержимого файлов. Клиентские события
ограничены 600 записями на пользователя в час, а ссылки на item/Flare проверяются
в текущем workspace; owner может проверить состояние очереди через
`GET /ops/queue` и сначала безопасно просмотреть очистку через
`POST /ops/queue/maintenance` с `dry_run: true`. Эта же операция ограниченно
удаляет activity events старше заданного срока (по умолчанию 90 дней).

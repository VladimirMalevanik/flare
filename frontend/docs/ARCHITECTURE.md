...
`ApiDataProvider` owns the HTTP boundary and validates DTOs before they reach the UI. `NEXT_PUBLIC_DATA_PROVIDER` selects it at build time. In API mode, versioned item writes, bounded text imports, the daily schedule/status, Analyze runs, Flares, analytics events, and GitHub connection operations use FastAPI. The provider delegates the base Sources catalog to `MockDataProvider`; API errors never become demo product data.
